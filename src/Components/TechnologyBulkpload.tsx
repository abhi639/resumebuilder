...
const TechnologyBulkupload = () => {
  ...
  const uploadFile = async (file: File) => {
    ...
    try {
      const response = await axios.post('http://localhost:8082/upload/TechnologyExcel', formData, {
    ...
    return acceptedFiles.map((file) => (
      <div key={file.name} style={{ marginTop: '5%' }}>
        ...
        <ProgressBar
          ...
          variant='success' // Set the color to green
        />
      </div>
    ))
  }
  ...
  const searchSettings: SearchSettingsModel = {
    fields: ['serialNumber', 'technology_name', 'status', 'remark'],
  }
  ...
  const handleDownloadTemplate = async () => {
    try {
      await dispatch(downloadFile()) // Wait for the downloadFile action to complete
    } catch (error) {
  ...
  return (
    <>
      <div style={gridContainerStyle}>
        <PageTitle>Bulk Upload | Technology</PageTitle>
...
export default TechnologyBulkupload
