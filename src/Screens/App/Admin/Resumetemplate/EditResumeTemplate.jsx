...
const EditResumeTemplate = (props) => {
  const location = useLocation()
  const template_id = location.state.iddata
  console.log('template_id', template_id.id)
  ...
  function onRichTextChange(editorName, content) {
    ...
    console.log('templateData set', templateData)
  }
  React.useEffect(() => {
    ...
    axios
      .get(`http://localhost:8082/templates/getTemplate/${template_id.id}`, config)
      ...
      .then((response) => {
        ...
        if (response.status === 200) {
          ...
          console.log('get templatedata', templateData)
        }
      })
      .catch((error) => {
...
saveSelection = selection.save(range, document);
  ...
  alert(`${nullFields} is empty `)
      ...
      axios
        .put(
          `http://localhost:8082/templates/updateTemplate/${template_id.id}`,
          templateData,
          config
        )
    ...
    React.useEffect(() => {
      const fetchData = async () => {
        try {
          ...
          const response = await axios.get('http://localhost:8082/placeholder/getPlaceholders', config);
    ...
    React.useEffect(() => {
      ...
      return (
  <>
      <PageTitle  children='Edit Template'description=''breadcrumbs=''></PageTitle>
    ...
    <tbody >
        ...
        <tr style={{height:'300px'}}>
          ...
          <td style={{width:'80%'}}> <RichTextEditorComponent ref={(richtexteditor) => {rteObj3.current = richtexteditor;}}width='100%' valueTemplate={templateData.projects} height='100%'id="defaultRTE"  change={(args) => onRichTextChange('projects', args.value)}toolbarSettings={toolbarSettings3}>
